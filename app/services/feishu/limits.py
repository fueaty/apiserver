...
#     · 量级参考（**归档实测**，非表内净增）：09-13 = 480、09-14 = 250
#       （读自 /opt 的 *_headlines_data.json 归档；旧注释写的 9/12=253、9/13=427
#        与实读不符，已按实读更正）。
#
# ⚠️ 度量口径（钉死，勿再用归档文件）：**净日增量 = 跑满 24h（含两次 cron）后表内总数
#    − 当天开始时表内总数**，必须从飞书表分页累加读取。容量不变式的正确输入是
#    **净日增量**，不是 gross 采集量——因为 _align 去重会把送出的量压缩。
#    现状：thepaper 曾「采到 100、入库 0」（写入形状缺陷，非采集问题）；
#    「计入 thepaper 后的实测净日增量」待 thepaper 入库修复后重测。
#    若实测净日增量 ≥ 680，则 25 天与 WATERMARK=17,000 不兼容，需**人工裁定**。
#    —— 在此之前，RETENTION_DAYS / DAILY_BUDGET / WATERMARK / WARNING 一律不动。
#
