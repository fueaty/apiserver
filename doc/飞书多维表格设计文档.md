...
> **关于 `published_at` 的语义说明（务必先读）**：该字段**当前并非全部是页面真实发布时间**。
> 仅有 `people_daily`、`tech_36kr` 两个站点从页面解析出真实 `pubdate`；其余 7 个站点
> （`baidu`、`cctv`、`thepaper`、`weibo`、`xiaohongshu`、`xinhua`、`zhihu`）写入的是**采集时刻的近似值**。
> 因此请勿据此字段做精确的时序判断。**「全站点真实发布时间抽取」属于后续独立需求，尚未实现。**
