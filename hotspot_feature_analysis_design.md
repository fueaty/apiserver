...
class FeishuDataLoader:
    ...
    def __init__(self):
        # 从配置中读取飞书表格信息
        self.app_token = "YOUR_APP_TOKEN_HERE"  # AI Headlines Pipeline
        self.table_id = "YOUR_TABLE_ID_HERE"
...
1. **数据获取阶段**：
   - 从飞书表格(app_token: "YOUR_APP_TOKEN_HERE", table_id: "YOUR_TABLE_ID_HERE")获取热点数据
   - 支持按日期过滤和排名排序
   - 选择需要分析的热点（默认top 10）
...
feature_analysis:
  # 飞书表格配置
  feishu:
    app_token: "YOUR_APP_TOKEN_HERE"  # AI Headlines Pipeline
