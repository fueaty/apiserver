...
def run():
    ...
    check("id == insight_id", fields['id'] == 'ins-0001', fields['id'])
    check("title 直传", fields['title'] == '热点标题', fields['title'])
    check("url 直传", fields['url'] == 'https://ex.com/a', fields['url'])
    check("author 直传", fields['author'] == '作者甲', fields['author'])
    check("content 取 headline.content", fields['content'] == '正文内容一段', fields['content'])
    check("published_at 取 headline", fields['published_at'] == '2025-01-01 12:00:00', fields['published_at'])
    check("status == 'generated'", fields['status'] == 'generated', fields['status'])

    # [3] tags / category
    print("\n[3] tags 与 category")
    check("tags 列表 -> 逗号文本", fields['tags'] == '热点, 科技, 观察', fields['tags'])
    check("category 取 LLM（在 generate_set 内）", fields['category'] == 'LLM分类', fields['category'])
    ...
    check("content 空 -> title+summary", '热点标题' in degraded['content'] and '摘要X' in degraded['content'],
          repr(degraded['content']))
    ws_content = dict(headline)
    ws_content['content'] = '   '
    degraded2 = build_insight_fields(ws_content, {'summary': '摘要Y'}, set(GENERATABLE), 'ins-0005')
    check("content 全空白也退化", '摘要Y' in degraded2['content'], repr(degraded2['content']))

    # [5] 缺失字段兜底为空串
    print("\n[5] LLM 缺字段 -> 空串")
    sparse = build_insight_fields(headline, {'summary': '仅有摘要'}, set(GENERATABLE), 'ins-0006')
    check("缺失 sentiment -> ''", sparse['sentiment'] == '', repr(sparse['sentiment']))
    check("缺失 seo_title -> ''", sparse['seo_title'] == '', repr(sparse['seo_title']))
    check("缺失 tags -> ''", sparse['tags'] == '', repr(sparse['tags']))
    check("仍有 14 键", len(sparse) == 14, len(sparse))

    # [6] tags 已是字符串
    print("\n[6] tags 字符串输入")
    str_tags = build_insight_fields(headline, {'tags': 'a, b'}, set(GENERATABLE), 'ins-0007')
    check("tags 字符串原样", str_tags['tags'] == 'a, b', repr(str_tags['tags']))
